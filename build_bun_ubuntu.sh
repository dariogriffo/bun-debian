...
build_package() {
    ...
    if ! docker build . -f "$dockerfile" -t "bun-ubuntu-$dist-$build_arch" \
    ...
    local id
    id="$(docker create "bun-ubuntu-$dist-$build_arch")"
...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        FULL_VERSION="$BUN_VERSION-${BUILD_VERSION}~${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        build_package bun-one one_Dockerfile.ubu "$dist" "$build_arch" "$FULL_VERSION" || return 1
        build_package bun-profile profile_Dockerfile.ubu "$dist" "$build_arch" "$FULL_VERSION" || return 1
        # bun: the meta package pointing at bun-one
        build_package bun meta_Dockerfile.ubu "$dist" "$build_arch" "$FULL_VERSION" || return 1
    done
